function [timeEM,Amps]=yr_of_timeGroupingByEyeMovement(vecX,vecY,timeOnset,sampleRate,msAmpThreshold,plotResults,trial_id, fr27, engbretThreshold, engbertMinDur)
...
%  Amps(1:num,3)   horizontal component     (dx)
%  Amps(1:num,4)   vertical component       (dy)
%  Amps(1:num,5)   horizontal amplitude     (dX)
%  Amps(1:num,6)   vertical amplitude       (dY)
%  Amps(1:num,7)   horizontal stability amp
%  Amps(1:num,8)   vertical stability amp
AmpsByAxes=[];
if (plotResults)
    a=1;
    for i=1:size(msMat,1)-1
        if (msMat(i,1)>fr27-timeOnset && msMat(i,1)<fr27-timeOnset+500)
            AmpsByAxes(end+1,1)=msMat(i,1)+timeOnset;
            AmpsByAxes(end,2)=msMat(i,2)+timeOnset;
            AmpsByAxes(end,3)=msMat(i,4);
            AmpsByAxes(end,4)=msMat(i,5);
            AmpsByAxes(end,5)=msMat(i,6);
            AmpsByAxes(end,6)=msMat(i,7);
            ampStabX=mean(vecX(msMat(i,2)+50:msMat(i,2)+100))-vecX(msMat(i,1));
            ampStabY=mean(vecY(msMat(i,2)+50:msMat(i,2)+100))-vecY(msMat(i,1));
...
for ms_id=1:size(AmpsByAxes,1)-1
    ...
    if AmpsByAxes(ms_id+1,1)-AmpsByAxes(ms_id,2)<=ceil(50./sampleRate)
        ...
        AmpsByAxes(i,7)=ampStabX;
        AmpsByAxes(i,8)=ampStabY;
...
%  Amps(1:num,1)   onset of saccade
%  Amps(1:num,2)   offset of saccade
%  Amps(1:num,3)   amp component     (dx)
%  Amps(1:num,4)   theta component       (dy)
%  Amps(1:num,5)   amp amplitude     (dX)
%  Amps(1:num,6)   theta amplitude       (dY)
%  Amps(1:num,7)   amp stability 
%  Amps(1:num,8)   theta stability 
Amps=[];
timeEM=[];
for ms_id=1:size(AmpsByAxes,1)
    Amp(ms_id,1)=AmpsByAxes(ms_id,1);
    Amp(ms_id,2)=AmpsByAxes(ms_id,2);
    Amp(ms_id,3)=sqrt(AmpsByAxes(ms_id,3).^2+AmpsByAxes(ms_id,4).^2);
    Amp(ms_id,4)=atand(AmpsByAxes(ms_id,4)./AmpsByAxes(ms_id,3));
    Amp(ms_id,5)=sqrt(AmpsByAxes(ms_id,5).^2+AmpsByAxes(ms_id,6).^2);
    Amp(ms_id,6)=atand(AmpsByAxes(ms_id,6)./AmpsByAxes(ms_id,5));
    Amp(ms_id,7)=sqrt(AmpsByAxes(ms_id,7).^2+AmpsByAxes(ms_id,8).^2);
    Amp(ms_id,8)=atand(AmpsByAxes(ms_id,8)./AmpsByAxes(ms_id,7));
