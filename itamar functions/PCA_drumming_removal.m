...
% plot_res(all_regions, Data_noBlinks, [100 100 10])
...
function selectedComponents = findTargetComponents(allTrialsPCA, referenceTrialPC)
    ...
    for t = 1:numTrials
        ...
        for c = 1:numComponents
            ...
            disp(correlations)
            if all(correlations > 0.75) % Adjust this threshold for sensitivity
...
function [location_on_mat_cell, resultsCell, regional_drum_components] = processBlocks(M_out, all_regions, blink_PC)
    ...
    for b = 1:numBlocks
        ...
        % Display or log some info
        fprintf('Processed Block ID = %d, #rows = %d\n', thisID, sum(rowsInBlock));
    end
