...
rawDataRoot='E:\Lab Files\raw_data\gandalf left\2018Dec25\b';
cortexFileRoot="E:\Lab Files\raw_data\gandalf left\2018Dec25\cortex files\gan_2018_12_25b.1";
...
while trialCortex_id<=size(trialsByCortex,1)&&trialCam_id<=size(trialsByCamera,1)
    if (trialsByCortex(trialCortex_id,2)~=0) %if there's error in cortex
        if trialsByCamera(trialCam_id)==8 %if appears also in camera, camera recorded error trial, for some monkeys replace with 7 instead of 8
...
trialsID=2:size(trialsSynched,2);trialsID(find(trialsSynched(1,:)==0))=[];
